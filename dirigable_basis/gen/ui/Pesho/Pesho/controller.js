angular.module('page', ["ideUI", "ideView", "entityApi"])
	...
	.controller('PageController', ['$scope', '$http', 'messageHub', 'entityApi', 'Extensions', function ($scope, $http, messageHub, entityApi, Extensions) {
		...
		Extensions.get('dialogWindow', 'dirigable_basis-custom-action').then(function (response) {
			$scope.pageActions = response.filter(e => e.perspective === "Pesho" && e.view === "Pesho" && (e.type === "page" || e.type === undefined));
		});
		...
		$scope.loadPage = function (pageNumber, filter) {
			...
			entityApi.count(filter).then(function (response) {
				if (response.status != 200) {
					messageHub.showAlertError("Pesho", `Unable to count Pesho: '${response.message}'`);
					return;
				}
				if (response.data) {
				...
				entityApi.search(filter).then(function (response) {
					if (response.status != 200) {
						messageHub.showAlertError("Pesho", `Unable to list/filter Pesho: '${response.message}'`);
						return;
					}
		...
		$scope.deleteEntity = function () {
			...
			messageHub.showDialogAsync(
				'Delete Pesho?',
				`Are you sure you want to delete Pesho? This action cannot be undone.`,
				[{
					id: "delete-btn-yes",
					type: "emphasized",
					label: "Yes",
				...
				if (msg.data === "delete-btn-yes") {
					entityApi.delete(id).then(function (response) {
						if (response.status != 204) {
							messageHub.showAlertError("Pesho", `Unable to delete Pesho: '${response.message}'`);
							return;
						}
						refreshData();
						$scope.loadPage($scope.dataPage, $scope.filter);
		...
		$http.get("/services/ts/dirigable_basis/gen/api/Settings/PeshoTypeService.ts").then(function (response) {
			$scope.optionsPeshoType = response.data.map(e => {
				return {
					value: e.Id,
					text: e.Name
				}
